...
- `2026-03-30`: Re-ran the clean `Orange` benchmark on the current workspace state and captured the full JSON report at `/tmp/checkmate-orange-bench/orange_w8_fresh.json`. Verified result: **220,679 hands, 8,288 tournaments, 8 workers, 1,441.4s e2e (24m 01.4s), 9,185.76 hands/min e2e, 153.10 hands/sec e2e**. Prepare wall time was `71.3s` (`4.95%` of e2e), runner wall time `1,370.1s`, and the aggregated runtime profile still shows a heavy finalize/materialize tail (`399.9s`) plus dominant DB persist cost (`5,746.7s` summed across workers). This rerun materially disagrees with the earlier `782.5s / 16,920 hands/min` entry, so benchmark comparisons now need an explicit commit/environment tag before treating older Orange numbers as authoritative.
