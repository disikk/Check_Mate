...
- Current persistence behavior:
  ...
  - HH persistence on the dir-import/runtime path is now collect-first and table-wise batched: one file job prepares canonical/normalized/derived rows outside the transaction, then a short DB transaction bulk-upserts `import.file_fragments` + `core.hands`, bulk-deletes child rows only for updated hands, and bulk-inserts child tables across the whole HH file instead of per-hand roundtrips;
  - fresh-hand detection now uses `RETURNING (xmax = 0) AS is_new` on `core.hands`, so the initial import path skips the old empty DELETE sweeps over child canonical/derived tables and keeps the full delete-and-replace path only for true re-imports;
...
- Current street-strength persistence behavior:
  ...
  - `tracker_parser_core::pot_resolution` no longer calls the full street-strength surface just to settle showdown winners; it now uses a dedicated lightweight `evaluate_river_showdown_ranks()` kernel and leaves the full derived `street_strength` evaluation to the later derived pass;
...
- `.zip` uploads and multipart batches with several files go through shared `tracker_ingest_prepare`, recursively expand nested ZIP members, enqueue only valid HH+TS leaf-pairs, and persist reject diagnostics into `import.ingest_events` through the web upload path before runtime execution;
...
- archive member addressing remains on the existing `member_path` DB/API contract, but nested ZIP leaf-files are now serialized as a stable locator chain and resolved recursively by both `tracker_ingest_prepare` and `parser_worker`;
- `parser_worker` now keeps a per-worker in-memory cache of opened archive bytes for repeated member reads, so `dir-import` no longer reparses the same large source ZIP or synthetic `prepared-pairs.zip` central directory on every job/member access;
- runtime archive input now also rejects text sources containing embedded NUL bytes as controlled terminal file failures instead of letting them reach PostgreSQL text writes;
...
- claim/success/fail runtime events no longer build payloads from a full bundle snapshot: `tracker_ingest_runtime` now emits bundle progress from a lightweight count summary and file updates from a single bundle-file/member status lookup, while the full snapshot load stays only for HTTP bundle reads and WebSocket bootstrap;
- the dependency-aware runner now splits file-job execution into `claim tx -> compute outside tx -> persist tx`, so parse/normalize/derive CPU work no longer keeps DB transactions and queue state rows open for the whole HH compute lifetime;
...
- Current stat runtime foundation:
  ...
  - `load_bundle_tournament_ids` in `mbr_stats_runtime::materializer` now joins through `source_file_member_id → import.file_fragments → core.hands.raw_fragment_id` instead of the old `source_file_id` join, which caused a combinatorial explosion for member-aware archives where all jobs share one parent `source_file_id`;
  - migration `0030_hands_source_file_index.sql` adds `idx_hands_source_file` on `core.hands(source_file_id)` for the legacy flat-file join path;
