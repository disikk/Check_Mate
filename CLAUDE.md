...
- Current persistence behavior:
  ...
  - `backend/migrations/0022_user_timezone_and_gg_timestamp_contract.sql` adds `auth.users.timezone_name`, strict GG timestamp provenance values (`gg_user_timezone | gg_user_timezone_missing`), and explicit unique upsert guards for `import.source_file_members(source_file_id, member_index)` and `import.file_fragments(source_file_id, fragment_index)`;
